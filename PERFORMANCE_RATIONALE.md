...
# Performance Optimization Rationale: Fast-Path for String Allocations in High-Frequency Loops

## Current State
In `Shared/PresenceInferenceEngine.swift`, the `incrementCount` function is called for every piece of evidence recorded during the presence inference pipeline. It normalized the processor ID using `let normalized = processorID.lowercased()` before performing string containment checks (e.g., `normalized.contains("stay")`).

## Problem
1. **O(N) Intermediate Allocation**: The `.lowercased()` function allocates a completely new String object on the heap every single time it is called.
2. **High-Frequency Execution**: `incrementCount` is a hot-path function executed thousands of times during inference (once for every location, photo, stay, and calendar signal, plus promotions and gap bridging). Allocating a new string for every execution introduces significant CPU time overhead and ARC (Automatic Reference Counting) thrashing.

## Optimization
Implemented a two-tier fast-path system before falling back to `.lowercased()`:
1.  A `switch` statement for known, statically-defined processor IDs (e.g., `"stay"`, `"photo"`, `"location"`, `"calendar"`, `"override"`, etc.) which provides an `O(1)` exact match check.
2.  An `if` statement using `.hasPrefix("Calendar")` to quickly catch dynamic promotion IDs generated by the calendar pipeline without allocation.

## Verification
-   **Correctness**: A standalone Python verification script confirmed that the fast-path logic maps exactly to the same `InferenceSourceCounts` properties as the original `.lowercased().contains(...)` logic for all known processor IDs.
-   **Time Complexity**: Reduced from `O(N)` string scanning and memory allocation to `O(1)` string hashing/comparison for the vast majority of operations.
-   **Space Complexity**: Eliminated the `O(N)` transient memory allocation inside the hot loop, reducing ARC overhead and garbage collection pressure.
