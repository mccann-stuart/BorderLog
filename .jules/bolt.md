...
## 2026-06-25 - Avoid O(N) allocation when calculating min and max in collections
**Learning:** Extracting dates from a generic array of structures to compute date ranges using `.map { $0[keyPath: keyPath] }` or `.flatMap { ... }` combined with `.min()` and `.max()` forces Swift to allocate a temporary O(N) intermediate array before performing any calculation, wasting memory and creating unnecessary GC thrashing.
**Action:** When finding minimum and maximum bounds over elements in an array or a mapped array, use a manual single pass O(N) `for` loop combined with O(1) local `min/max` tracking variables. This provides identical algorithmic performance while entirely bypassing the heap allocation costs.
