...
## 2026-03-21 - In-Place Array Mutation for Struct Collections
**Learning:** In high-frequency pipelines (like `PresenceInferenceEngine`), using `.map` to iterate over an array of structs (e.g. `[PresenceEvidenceEntry]`) to modify a single property forces the creation of a completely new array and subsequent full-struct initializations. This leads to severe O(N*M) heap allocation and ARC (Copy-on-Write) overhead.
**Action:** When mutating arrays of nested structs, change the containing array to a `var` (e.g. `var evidenceEntries`) and use an in-place `for` loop with index-based access (`array[i].property = true`) and `inout` parameters instead of returning a new array via `.map`.
