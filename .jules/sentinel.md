...
## 2026-06-02 - Keychain Passcode Enforcement
**Vulnerability:** Keychain items were accessible simply when the device was unlocked via `kSecAttrAccessibleWhenUnlockedThisDeviceOnly`, which could expose data if a user removed their device passcode entirely.
**Learning:** Apple's Security framework does not have a `WhenPasscodeSet` constant. To ensure data is cryptographically tied to the presence of a device passcode and wiped if the passcode is removed, `kSecAttrAccessibleWhenPasscodeSetThisDeviceOnly` must be used.
**Prevention:** Always use `kSecAttrAccessibleWhenPasscodeSetThisDeviceOnly` for sensitive authentication or session tokens in Keychain to enforce device-level security.
