...
## 2026-04-19 - Production Authentication Bypass Vulnerability
**Vulnerability:** A static boolean flag (`isAppleSignInEnabled`) was used to conditionally enable a local-only mock authentication flow that generated a random UUID and bypassed the mandatory Apple Sign-In flow entirely. This bypass logic was inadvertently left in the production source code and UI.
**Learning:** Any form of mock or local-only authentication bypass in production code violates access control. Do not ship code capable of skipping authentication flows (e.g., using explicit toggles or local UUID spoofing), even for internal debugging. Hardcode required authentications without bypass mechanisms.
**Prevention:** Completely remove authentication bypass flags and the associated mock logic prior to deployment. Rely solely on the standard production identity provider (e.g., Apple Sign-In) for authentication flows.
