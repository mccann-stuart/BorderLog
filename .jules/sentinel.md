...
## 2026-04-03 - Authentication Bypass via Local-Only Toggle
**Vulnerability:** A mock local-only authentication bypass was present in production code (`isAppleSignInEnabled` toggle), allowing users to skip Apple Sign-In and access the app with a mock UUID.
**Learning:** Development conveniences (like bypassing auth) left in production code are critical access control vulnerabilities. They provide an easy vector for unauthorized access.
**Prevention:** Never ship code capable of skipping authentication flows (e.g., using explicit toggles or local UUID spoofing), even for internal debugging. Hardcode required authentications without bypass mechanisms.
