...
actor CalendarTabDataService {
    ...
    func snapshot(
        ...
        var accumulators = fetchDayKeys.reduce(into: [String: DayAccumulator](minimumCapacity: fetchDayKeys.count)) { $0[$1] = DayAccumulator() }
        ...
        let configByID = countryConfigs.reduce(into: [String: Int?](minimumCapacity: countryConfigs.count)) { result, config in
            if result[config.countryCode] == nil {
