...
// Helper to assert all security headers
function assertSecurityHeaders(res) {
  ...
  assert.strictEqual(res.headers.get("Strict-Transport-Security"), "max-age=31536000; includeSubDomains; preload", "Missing HSTS");
  assert.strictEqual(res.headers.get("X-Frame-Options"), "DENY", "Missing X-Frame-Options");
  assert.strictEqual(res.headers.get("Content-Security-Policy"), "default-src 'none'; frame-ancestors 'none';", "Missing CSP");
  assert.strictEqual(res.headers.get("Referrer-Policy"), "no-referrer", "Missing Referrer-Policy");
  assert.strictEqual(res.headers.get("Permissions-Policy"), "interest-cohort=()", "Missing Permissions-Policy");
}
