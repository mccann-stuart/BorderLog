...
- Sign in with Apple is disabled by `AuthenticationManager.isAppleSignInEnabled == false`; version 1.0 is account-free and uses a device-local session ID.
...
- `SettingsView.swift`: permissions, privacy controls, calendar-source selection, reset, feature toggles, shared day-counting mode, and debug-only export.
