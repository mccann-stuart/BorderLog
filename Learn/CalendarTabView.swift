...
struct CalendarTabView: View {
    ...
    private func summaryPresenceDayKeys(from snapshot: CalendarTabSnapshot) -> [String] {
        // ⚡ Bolt: Avoid intermediate array allocation by using .lazy.map when converting to a Set.
    ...
    private func loadPresenceDays(for dayKeys: [String]) {
        ...
        do {
            ...
            presenceDaysByKey = days.reduce(into: [String: PresenceDay](minimumCapacity: days.count)) { $0[$1.dayKey] = $1 }
        } catch {
            presenceDaysByKey = [:]
        }
    }
